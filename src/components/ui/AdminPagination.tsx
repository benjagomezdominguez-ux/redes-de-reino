...
export async function AdminPagination({
  ...
  pageSize: number;
  total: number;
  // Preserved across prev/next links (e.g. the users list's status
  // filter) so paginating never silently resets an active filter.
  ...
  return (
    <nav aria-label={t("label")} className="flex items-center justify-between pt-2">
      <Link
        ...
        href={{ pathname: basePath, query: { ...extraQuery, page: String(Math.max(1, page - 1)) } }}
      ...
      <Link
        ...
        href={{ pathname: basePath, query: { ...extraQuery, page: String(Math.min(totalPages, page + 1)) } }}
        className={`rounded-full border border-border px-4 py-2 text-sm font-medium text-primary-900 transition-colors ${
