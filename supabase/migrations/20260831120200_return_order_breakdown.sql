-- Same function, same signature — just returns subtotal/tax alongside
-- the total so the checkout UI can show "base + impuestos = total"
-- explicitly (rule 9/13) using the one server-computed number, not a
