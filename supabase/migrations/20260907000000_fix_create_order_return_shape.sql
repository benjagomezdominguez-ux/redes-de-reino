-- Found while splitting the checkout flow (see the two migrations right
-- after this one): create_order()'s returned jsonb only ever carried
-- order_id/reference/total_cents/payment_method — subtotal_cents,
-- tax_cents and currency were added once (20260831120200) but silently
-- dropped again the very next time the function was redefined
-- (20260901180000, for the ARS currency switch, which only touched the
-- currency logic and reused the older 4-key return by mistake). The
-- practical effect: CheckoutView.tsx's success screen has been showing
-- $0.00 subtotal and $0.00 tax on every real order since. Same function
-- body otherwise — only the final return adds the three keys back.
